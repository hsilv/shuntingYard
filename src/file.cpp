...
void generateFile(std::map<wstring, wstring> rules, const wstring &filename)
{
    // Convert wstring to string for ofstream
    std::string filenameStr(filename.begin(), filename.end());

    // Create and open a text file
    std::ofstream MyFile(filenameStr);
...
Automata *configYalexAutomata()
{
    ...
    {
        std::string rule(pair.second.begin(), pair.second.end());
        std::string type(pair.first.begin(), pair.first.end());
        rawString += "    automatas = addRegex(automatas, L\"" + rule + "\", L\"" + type + "\");\n";
    }

    MyFile << rawString;
    MyFile << R"~( return joinAutomatas(automatas);
}
